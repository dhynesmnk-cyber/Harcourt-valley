...
import { fmtDate, type OutboxItem } from "../../lib/data";
...
export function OutreachView() {
  const { profiles, addProfile, addOutbox, outbox, setOutboxState, leads, toast } = useStore();
  const [newName, setNewName] = useState("");
  const [newWho, setNewWho] = useState("");
  const [profileErr, setProfileErr] = useState("");
  const [analyzing, setAnalyzing] = useState(false);
  const [analysisStage, setAnalysisStage] = useState(0);
  const [results, setResults] = useState<{ criteria: string[]; suggestions: Suggestion[]; profileName: string } | null>(null);
          ...
          <div className="mt-6 rise-in">
            ...
            <div className="mt-5 grid sm:grid-cols-3 gap-3">
              {results.suggestions.map((s) => (
                <div key={s.business} className="border border-granite-500 p-4 bg-granite-700/40">
                  <p className="font-label font-semibold text-sm">{s.business}</p>
                  <p className="text-xs text-granite-300 mt-0.5">
                    {s.town} · ask for {s.contact}
                  </p>
                  <button type="button" className="btn btn-sm bg-bone text-granite-900 mt-3 w-full" onClick={() => generateDraft("bp2", s)}>
                    Write the email
                  </button>
                </div>
              ))}
        ...
        <div className="mt-3 grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          {profiles.map((p) => (
            <div key={p.id} className="border-2 border-granite-900 bg-bone p-5">
              ...
              <button type="button" className="btn btn-ghost btn-sm mt-4" onClick={() => setResults({ profileName: `Matches for "${p.name}"`, criteria: p.criteria, suggestions: SUGGESTIONS[p.id] ?? SUGGESTIONS.bp1 })}>
